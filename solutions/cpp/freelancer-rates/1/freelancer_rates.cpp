...
int monthly_rate(double hourly_rate, double discount)
{
    ...
    // discount.
    double monthly_total = 22 * daily_rate(hourly_rate);
    double after_discount = monthly_total - (monthly_total * (discount / 100));
    return ceil(after_discount);
}
...
int days_in_budget(int budget, double hourly_rate, double discount)
{
    ...
    return floor(budget / after_discount);
}
